...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def check(string):
        string_list = split(remove_punctuation(lower(string)))
        for j in range(0, len(string_list)):
            for i in range(0, len(topic)):
                if  string_list[j] == topic[i]:
                    return True
        return False
    return check
    # END PROBLEM 2
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    diff = min(valid_words, key = lambda x: diff_function(user_word, x, limit))
...
def swap_diff(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def diff(start, goal, count, limit):
        if limit < 0:
            return limit + 1
        if count == len(start) or count == len(goal):
            return abs(len(goal) - len(start))
        if start[count] == goal[count]:
            return diff(start, goal, count + 1, limit)
        else:
            return 1 + diff(start, goal, count + 1, limit - 1)
    return diff(start, goal, 0, limit)
    # END PROBLEM 6

def edit_diff(start, goal, limit):
    """A diff function that computes the edit distance from START to GOAL."""
    if start == goal or start == '' or goal == '': # Fill in the condition
        # BEGIN
        "*** YOUR CODE HERE ***"
        return abs(len(goal) - len(start))
        # END
...
def fastest_words(word_times, margin=1e-5):
    ...
    for i in range(n_players):
        ...
        for j in range(1, len(word_times[i])):
            typed_word = word(word_times[i][j])
            time = elapsed_time(word_times[i][j])
            time_to_type = time - elapsed_time(word_times[i][j-1])
            time_list.append(time_to_type)
            typed_list.append(typed_word)
        if n_players == 1:
            return [typed_list]
        player_time_dict.update({i: time_list})
        word_dict.update({i: typed_list})
    final_word_list = []
    for i in range(n_players):
        word_array = []
        for j in range(len(player_time_dict[0])):
            for k in range(n_players):
                if k == i:
                    continue
                value = True
                if player_time_dict[i][j] < player_time_dict[k][j] or abs(player_time_dict[i][j] - player_time_dict[k][j]) <= margin:
                    continue
                else:
                    value = False
                    break
            if value:
                word_array.append(word_dict[i][j])
        final_word_list.append(word_array)
